class pt { float x=0,y=0; 
  ...
  pt label(String s, float u, float v) {fill(0,0,0); text(s, x+u, y+v); noFill(); return this; };
  pt label(String s, vec V) {fill(0,0,0); text(s, x+V.x, y+V.y); noFill(); return this; };
...
pt P(pt P, float u, vec U, float v, vec V) {return P(P.x+u*U.x+v*V.x, P.y+u*U.y+v*V.y); }            //  P+sV (P transalted by sV)
pt T(pt P, float s, vec V) {return P(P,W(s,V)); }                                                    //  P+sV (P transalted by sV)
pt MoveByDistanceTowards(pt P, float d, pt Q) { return P(P,d,U(V(P,Q))); };                          //  P+dU(PQ) (transLAted P by *distance* s towards Q)!!!
pt T(pt P, float d, pt Q) { return MoveByDistanceTowards(P,d,Q); };                                  // T(P,s,Q): P+sU(PQ) (transalted P by distance s towards Q)
...
vec slerp(vec U, float t, vec V) {float a = angle(U,V); float b=sin((1.-t)*a),c=sin(t*a),d=sin(a); return W(b/d,U,c/d,V); } // UNIT vectors ONLY!
...
float det(vec U, vec V) {return U.x*V.y-U.y*V.x; }
